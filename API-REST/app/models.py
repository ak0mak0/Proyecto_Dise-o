...
class Sitio:
    ...
    def get_reviews(self):
        ...
        reviews = reviews_collection.find({"id_sitio": self._id})
        return list(reviews)
...
class Review:
    def __init__(self, usuario, sitio):
        ...
        self.id_usuario = ObjectId(usuario._id)
        self.id_sitio = ObjectId(sitio._id)
        self.opinion = ""
        self.calificacion = 6
        self.like = False
        self.visito = False
...
# CLASE CON METODOS PARA MANIPULAR LAS RECOMENDACIONES
class Recomendaciones:
    def __init__(self, mongodb_manager, sitio_id, user_id = None):
        self.db_manager = mongodb_manager
    ...
    def __find_similar_sites(self):
        ...
        for otro_sitio in self.sitios_collection.find({"_id": {"$ne": self.sitio["_id"]}}):
    ...
    def get_sitios_cercanos(self):
        db = self.db_manager.get_db()
        recos_sitios_collection = db["recos_sitios"]
        sitio = recos_sitios_collection.find_one({"_idsitio": self.sitio_id})
        if sitio:
...
# CLASE CON METODOS PARA MANIPULAR LAS CATEGORIAS
class Categoria:
